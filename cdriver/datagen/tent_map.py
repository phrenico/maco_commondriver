...
class TentMapExpRunner:
  ...
  def sample_params(self, A=None, a=None, x0=None, min_c_strength=0.1, seed=None):
    np.random.seed(seed)
    ...
    if A is None:
      A = self.baseA * ( (1 - min_c_strength) * np.random.rand(self.nvars**2).reshape([self.nvars, self.nvars]) + min_c_strength )
  ...
  def gen_experiment(self, n, A=None, a=None, x0=None, min_c_strength=0.1, seed=None):
    a, A, x0 = self.sample_params(a=a, A=A, x0=x0, min_c_strength=min_c_strength, seed=seed)
    data = TentMap(alpha=a, A=A, x0=x0).gen_dataset(n)
    self.data = data
    return data, {'a': a, 'A': A, 'x0': x0}

def gen_tentmapdata(tentmapgen_params):
  p = SimpleNamespace(**tentmapgen_params)
  ...
  dataset, params = zip(*[TentMapExpRunner(nvars=3,
                                   baseA=p.A0,
                                   a_interval=p.aint).gen_experiment(n=p.n,
                                                                   seed=base_seed + i) for i in tqdm(range(p.N),
                                                                                         desc='Generating TentMap Data')])
  return dataset, params

# Canonical tent-map data generation: handled by experiment scripts via gen_tentmapdata()
