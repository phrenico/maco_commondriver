...
def _get_default_paths(config_path=None):
    if config_path is not None:
        cfg = resolve_paths(get_config('logmaps', config_path), _REPO_ROOT)
        paths = cfg.get('paths', {})
        final_res_path = paths['final_res_path']
        figure_path = paths.get('figure_path', figures_root)
        return final_res_path, figure_path

    from scripts.config_runall import CONFIG_LOGMAPS

    paths = CONFIG_LOGMAPS.get('paths', {})
    if 'final_res_path' not in paths:
        raise KeyError("CONFIG_LOGMAPS['paths']['final_res_path'] is required")
...
def plot_logmap_comparison(final_res_path, figure_path):
    ...
    plt.tight_layout()
    plt.savefig(figure_path / 'comparison_logmap.png', dpi=300)
    return fig
...
def main(args=None, config_path=None, final_res_path=None, figure_path=None):
    ...
    return plot_logmap_comparison(final_res_path=final_res_path,
