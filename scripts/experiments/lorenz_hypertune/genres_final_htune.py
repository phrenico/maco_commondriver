import pandas as pd
from scripts.experiments.lorenz_hypertune.htune_config import interim_save_path, final_save_path, final_savefig_path
...
kpca_df = pd.read_csv(interim_save_path / './kpca_htune.csv', index_col=0)
...
df = pd.concat([pca_df, kpca_df, ica_df, dca_df, sfa_df], ignore_index=True, axis=0)
df.to_csv(final_save_path / 'htune.csv')

fig, ax = plt.subplots(2, 5, figsize=(25, 10), sharex=True, sharey='row')
plot_htune(pca_df, 'PCA', fig_axes=[fig, ax[0, 0], ax[1, 0]],
           yaxlabel=True, save=False)
plot_htune(kpca_df, 'kPCA', fig_axes=[fig, ax[0, 1], ax[1, 1]],
           yaxlabel=False, save=False)
plot_htune(ica_df, 'ICA', fig_axes=[fig, ax[0, 2], ax[1, 2]],
           yaxlabel=False, save=False)
plot_htune(dca_df, 'DCA', fig_axes=[fig, ax[0, 3], ax[1, 3]],
           yaxlabel=False, save=False)
plot_htune(sfa_df, 'SFA', fig_axes=[fig, ax[0, 4], ax[1, 4]],
           yaxlabel=False, save=False)

# Add A, B, C, D, E, F , G, H labels to subplots
for i, label in enumerate(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']):
    ax[i//5, i%5].text(-0.1, 1.1, label, transform=ax[i//5, i%5].transAxes,
...
ax[0, 3].text(4, 1, **star_kwargs)
ax[0, 4].text(2, 1, **star_kwargs)
