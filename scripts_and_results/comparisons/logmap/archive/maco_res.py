'''Apply MaCo to the random Logistic datasets (less epochs [100], bigger batch size [2_000])
0. Import packages
...
def split_sets(Q, Targ, z, trainset_size, testset_size, validset_size):
    """

    :param Q: input data
    :param Targ: target dataq
    ...
    Q_trainset = Q[:n_trainset]
    Q_testset = Q[n_trainset:n_trainset + n_testset]
    Q_validset = Q[n_trainset + n_testset:]

    T_trainset = Targ[:n_trainset]
    T_testset = Targ[n_trainset:n_trainset + n_testset]
    T_validset = Targ[n_trainset + n_testset:]

    z_trainset = z[:n_trainset]
    z_testset = z[n_trainset:n_trainset + n_testset]
    z_validset = z[n_trainset + n_testset:]
    return ((Q_trainset, T_trainset, z_trainset),
            (Q_testset, T_testset, z_testset),
            (Q_validset, T_validset, z_validset))
...
for i in tqdm(range(N)):
    ...
    train_loader, test_loader, _, z_test = get_loaders(data, batch_size=2000, trainset_size=50,
...
# Save results
df = save_results(fname='./maco_res.csv', r=maxcs, N=N, method='MaCo', dataset='logmap_fixed')
